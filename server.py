...
def thing():
    ...
    metrics['voice_score'] = (metrics['voiced_prob'] - 0.461134) / 0.140464 + 0.3 * (
            4.012101 - metrics['mosnet']) / 0.355457
    # return jsonify(metrics)
    return render_template('index.html',
                           pitch_score=round(metrics['pitch_score'], 2),
                           voice_score=round(metrics['voice_score'], 2),
                           voiced_prob=round(metrics['voiced_prob'] * 100, 2),
                           pitch=round(metrics['f0'], 2),
                           stddev=round(metrics['stddev'], 2),
                           syllable_estimate=round(metrics['syllable_estimate'], 2),
                           voiced_percent=round(metrics['voiced_percent'] * 100, 2),
                           srmr=round(metrics['srmr'], 2),
                           mosnet=round(metrics['mosnet'], 2),
                           lengthfile=round(metrics['length'], 2),
                           filename=metrics['filename'],
                           scroll='scroll'
                           )


@app.route('/thing2', methods=['POST'])
def thing2():
    f = request.files['file']
    f.save(f.filename)  # Saves the file
    wav_filename = 'tmp.wav'
    ...
    return jsonify((round(metrics['pitch_score'], 2),
                    ...
                    round(metrics['srmr'], 2),
                    round(metrics['mosnet'], 2),
